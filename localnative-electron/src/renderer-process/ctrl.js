...
window.ctrl = (function() {
  ...
  function onNativeMessage(message) {
    ...
    if (Number(message.count) >= 0) {
      document.getElementById('pagination-text').innerHTML = window.appState.makePaginationText();
    }
    refreshNotes(message.notes);

    if (message.days && window.appState.getOffset() == 0) {
      window.chart.refreshChart(message.days);
    }
  }
