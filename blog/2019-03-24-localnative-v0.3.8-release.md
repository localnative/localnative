...
- Currently, Local Native desktop implemented wired sync based on files via exchange and appending items from one to another and vice versa.
- Also, Local Native desktop implemented ssb sync by setting up target keys.
...
- On iOS the tags layout is not optimal as it does not scroll and each tag is same width regardless of character length.
- Crash when adding note on Nexus 5X. (on Pixel 2 and Nexus 6 is fine)
- Mac app store build has sandbox issue so it is not quite usable. ([DMG](https://gitlab.com/yiwang/localnative-release/tree/master/v0.3.8/mac) works)
- ...
