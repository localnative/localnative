...
impl SettingView {
    ...
    pub fn viwe(&mut self, server_state: bool) -> Element<Message> {
        let SettingView { config, state, .. } = self;
        left_bar_viwe(state, config.theme, server_state)
    }
    pub fn setting_board_open_view(&mut self, server_state: bool) -> Element<Message> {
        let SettingView {
            config,
            state,
            board_state,
        ...
        let left_bar = left_bar_viwe(state, config.theme, server_state);
        let setting_board = setting_board_view(board_state, &*config);
        Row::new().push(left_bar).push(setting_board).into()
    }
    pub fn sync_board_open_view(&mut self, server_state: bool) -> Element<Message> {
        let SettingView {
            config,
            state,
            sync_state,
...
fn left_bar_viwe(state: &mut State, theme: Theme, server_state: bool) -> Element<Message> {
    let State {
        theme_button,
        setting_button,
        server_button,
